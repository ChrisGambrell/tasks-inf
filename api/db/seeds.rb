...
task1 = Task.create(header_id: 1, project_id: 2, title: 'Double-click this to-do', notes: 'You’re looking at a to-do! Complete it by clicking the checkbox on the left. Completed to-dos are collected at the bottom of your project.',	completed: false, completed_when: nil,	when: Time.current)
task2 = Task.create(header_id: 1, project_id: 2, title: 'Create a new to-do', notes: 'Your turn – just hit ⌘N on your keyboard.', completed: false, completed_when: nil, when: Time.current + 1.day)
task3 = Task.create(header_id: 1, project_id: 2, title: 'Add this to-do to your Today list', notes: 'Click the calendar button below to plan when you’ll do this to-do. Choose Today.',	completed: true, completed_when: Time.current - 1.day,	when: Time.current)
...
task5 = Task.create(header_id: 1, project_id: 2, title: 'Create a new heading', notes: 'Go to File → New Heading to create a heading, then drag this to-do under it.', completed: true, completed_when: Time.current, when: Time.current + 6.days)
task6 = Task.create(header_id: 1, project_id: 2, title: 'Create a project', notes: 'On to bigger things! At the bottom of the sidebar, click “+ New List” to add a project of your own.', completed: true, completed_when: Time.current - 4.days, when: Time.current + 10.days)
task7 = Task.create(header_id: nil, project_id: 2, title: 'Task without header & without description', notes: nil, completed: false, completed_when: nil, when: Time.current + 1.month)
...
task10 = Task.create(header_id: 2, project_id: 2, title: 'Show your calendar events', notes: 'You can connect your calendars to Things and see all of your events and to-dos together. Go to Things → Preferences → Calendar Events to turn it on.', completed: false, completed_when: nil, when: Time.current + 4.months)
task11 = Task.create(header_id: 2, project_id: 2, title: 'Add some widgets', notes: 'Widgets let you quickly access to your lists while working in other apps. You can add as many as you like. Here’s how:', completed: true, completed_when: Time.current - 1.year - 6.months, when: Time.current + 4.months + 2.days)
